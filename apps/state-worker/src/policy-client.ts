// Internal call to policy-worker: deny-by-default authorization for the
// workspace-link surface (state-api-contract §6 — `org.cli.link`). Fails
// closed: any error or non-allow decision returns `{ allow: false }`.
...
export async function authorizeViaPolicy(
  ...
  if (!parsed || typeof parsed !== "object" || !("data" in parsed)) return { allow: false };
