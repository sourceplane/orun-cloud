// GET /ingress/supabase/oauth — the OAuth-callback half of the Supabase
// tenancy keystone (IH6, design §5.3). Supabase redirects the installing user
// here with a `code` and (always, for flows we initiated) the signed state.
// The org binding comes ONLY from our state — `supabase_org_id ↔ org_id` is
...
//                            (`/v1/oauth/token`) → refresh-token custody +
//                            org facts → activate (+ event)
//   anything else          → fail popup
//
// Unlike Slack there is NO orphan-record path: the PKCE exchange cannot run
...
const LINK_FAILED_MESSAGE =
  "We couldn't link this Supabase organization to an organization. Start the connection again from your organization's Integrations settings.";

/** Test seam: inject a fake executor / Supabase fetch; production omits it. */
export interface SupabaseOauthDeps {
  executor?: SqlExecutor;
  fetchImpl?: FetchLike;
}

export async function handleSupabaseOauthCallback(
  request: Request,
  env: Env,
  requestId: string,
  deps?: SupabaseOauthDeps,
): Promise<Response> {
  if (!env.PLATFORM_DB) {
  ...
  // The user declined the Supabase consent screen — nothing was granted, the
  // pending connection simply expires with its state.
  if (providerError) {
    return popupPage(
      "error",
      "Connection not completed",
      "Supabase reported the authorization was cancelled.",
    );
  }
  if (!code) {
  ...
  const configured = getConfiguredProvider(env, "supabase", deps?.fetchImpl);
  ...
  if (!configured || !redirectBase || !encryption) {
    return popupPage(
      "error",
      "Not configured",
      "The Supabase OAuth app for this environment is not configured yet.",
    );
  }
  // The registry gate above guarantees both secrets exist.
  const credentials = {
    clientId: env.SUPABASE_OAUTH_CLIENT_ID!,
    clientSecret: env.SUPABASE_OAUTH_CLIENT_SECRET!,
  };
  const redirectUri = `${redirectBase}${SUPABASE_OAUTH_CALLBACK_PATH}`;
  ...
  try {
    ...
    // No orphan record: without the state there is no connection, hence no
    // PKCE verifier, hence nothing the exchange could even run with.
    const fail = (): Response =>
    ...
    await hub.deleteProviderCredential(asUuid(connection.id), "supabase_pkce_verifier");
    ...
    // Exchange the code with the verifier — Supabase verifies the code, the
    ...
    if (!grant) {
      return popupPage(
        "error",
        "Verification failed",
        "Supabase did not confirm this authorization. Try connecting again.",
      );
    }

    // The organization behind the grant — the connection's external anchor.
    const org = await discoverSupabaseOrg(grant.accessToken, deps?.fetchImpl);
    if (!org) {
      return popupPage(
        "error",
        "Verification failed",
    ...
    // Verified facts, project list best-effort (connect proceeds without it).
    const projects = await listSupabaseProjects(grant.accessToken, deps?.fetchImpl);
    const facts = await hub.upsertSupabaseOrg({
      id: generateUuid(),
      connectionId: asUuid(connection.id),
      supabaseOrgId: org.supabaseOrgId,
      orgName: org.orgName,
    ...
    // The org row must end up bound to THIS connection — an org already
    ...
    const activated = await repo.activateConnection(
      ...
      asUuid(connection.id),
      {
        displayName: connection.displayName ?? org.orgName ?? org.supabaseOrgId,
        externalAccountLogin: org.orgName ?? org.supabaseOrgId,
        externalAccountId: org.supabaseOrgId,
    ...
    if (!activated.ok) {
      if (activated.error.kind === "conflict") {
        return popupPage(
          "error",
          "Already connected",
          "An active connection for this Supabase organization already exists in the organization.",
        );
      }
      return popupPage("error", "Connection not completed", LINK_FAILED_MESSAGE);
    }
    ...
    try {
      ...
      await events.appendEventWithAudit({
        ...
        audit: {
          id: generateUuid(),
          category: "integrations",
          description: `Supabase connected${org.orgName ? ` (${org.orgName})` : ""}`,
        },
      });
    } catch {
