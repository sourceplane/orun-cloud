...
// Event-groups facade (saas-event-streaming ES4): read-only dedup/correlation
// stories, forwarded to events-worker. Same GET-only shape as the audit
// facade; authorization (organization.event.read) happens downstream.
...
export function isEventGroupsRoute(pathname: string): boolean {
  return ORG_EVENT_GROUPS_RE.test(pathname) || ORG_EVENT_GROUP_RE.test(pathname);
}

export async function handleEventGroupsRoute(
  request: Request,
  env: Env,
  requestId: string,
  pathname: string,
  ...
  try {
    const downstream = await timings.measure("edge_downstream", () =>
      env.EVENTS_WORKER!.fetch(target.toString(), { method: "GET", headers }),
    );
    endTotal();
    return mergeRateLimitHeaders(
      withEdgeTimings(
