...
export async function handleOrgRoute(
  ...
  if (ORG_TEAM_GRANTS_RE.test(pathname) && request.method !== "GET") {
    return errorResponse("unsupported", "Method not allowed", 405, requestId);
  }
  ...
  if (ORG_OWNER_HANDLES_RE.test(pathname) && request.method !== "GET" && request.method !== "PUT" && request.method !== "POST") {
    return errorResponse("unsupported", "Method not allowed", 405, requestId);
  }

  if (ORG_OWNER_HANDLE_ID_RE.test(pathname) && request.method !== "DELETE") {
    return errorResponse("unsupported", "Method not allowed", 405, requestId);
  }

  if (ORG_RESOLVE_OWNERS_RE.test(pathname) && request.method !== "POST") {
    return errorResponse("unsupported", "Method not allowed", 405, requestId);
  }

  if (ORG_MY_TEAMS_RE.test(pathname) && request.method !== "GET") {
    return errorResponse("unsupported", "Method not allowed", 405, requestId);
  }
  ...
  return replayOrExecute(request, requestId, env, "org", async () => {
    ...
    if (
      request.method === "POST" ||
      request.method === "PATCH" ||
      // teams-ownership TO1 — setting an owner-handle alias is an idempotent PUT.
      request.method === "PUT" ||
