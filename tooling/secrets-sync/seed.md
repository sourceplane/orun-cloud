...
| `…/integrations/slack-app/<env>` | `SLACK_APP_CLIENT_ID`, `SLACK_APP_CLIENT_SECRET`, `SLACK_APP_SIGNING_SECRET` — **deferred** until the Slack App is registered per environment (IH risks D1); clear the `slack-app` doc's `deferred` flag when seeding |
