...
import { API_VERSION, WorkError, type Contract, type CoordinationEvent, type Spec, type Task } from "./model.js";
...
export function buildEnvelopes(workspace: string, events: CoordinationEvent[]): Envelopes {
  ...
  for (const e of events) {
    switch (e.kind) {
      ...
      default:
        // assigned/unassigned/comment_added/ordered/pinned/canceled carry
        // coordination state the fold reads from the log directly — they
        // never touch the intent envelope.
        break;
    }
  }
