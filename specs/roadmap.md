...
| **TM** | [`epics/saas-teams/`](./epics/saas-teams/) | Draft | Account-owned **Teams as principals**: TM1 model (`teams`+`team_members`) · TM2 grants via `role_assignments` (`subject_type='team'`) · TM3 authz-context fact expansion (engine unchanged) · TM4 management surfaces + `team.*` RBAC · TM5 PERF2 cache invalidation. Account-scope grants cascade to all (incl. future) workspaces. Principal-group, **not** a hierarchy level. Depends on **WID6**. |
