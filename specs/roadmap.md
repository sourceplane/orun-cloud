...
| **IC** | [`epics/saas-instant-console/`](./epics/saas-instant-console/) | Draft | IC0–IC9 product-feel ladder from the 2026-07-23 full-surface audit: IC1 `/state/runs` N+1 · IC2 boot dedupe · IC3 paint-from-cache · IC4 perceived speed · IC5 digest-doc caching · IC6 stream discipline · IC7 data-backed ⌘K (delivers PX6) · IC8 list hygiene · IC9 CI budgets. Complements PERF (request path) with the surface experience. |
...
| **IR** | [`epics/saas-integration-registry/`](./epics/saas-integration-registry/) | ✅ Shipped (2026-07-23, #596–#602 + orun #559) | **One registry, one page per integration, every plane derived** — unifies the IH capability seam, the SP secrets ownership, and the AG provider panel into a single `IntegrationManifest` registry: IR0 manifest + bulk registry read (secrets-capabilities becomes a projection) · IR1 unified hub (AI/compute cards join; special-cases die) · IR2 canonical `/integrations/{provider}` space + nested connection detail + redirects · IR3 Cloudflare unification (connect-in-space, multi-account, single-source scope grammar) · IR4 outcome-first secret wizard · IR5 AI+Daytona connection re-home (`agents.provider_connections` → facts table; custody untouched) · IR6 provider modules (repos/channels/projects/models/sandboxes) · IR7 registry-served CLI verb trees (pairs `orun/specs/orun-integrations-cli/`) · IR8 manifest governance + docs generation · IR9 pluggability proof. |
