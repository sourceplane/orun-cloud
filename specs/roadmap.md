...
| **SI** | [`epics/saas-settings-ia/`](./epics/saas-settings-ia/) | Draft | Console **information architecture** — make navigation mirror the shipped `You → Account → Workspace` scope model: SI1 re-file the four mis-scoped surfaces (Billing→Account · Sessions/personal-notifs→You · keep event-routing at Workspace · de-orphan Config) · SI2 three doorways (settings follow the switcher; hosts the `teams-hub` TH1 Account Hub) · SI3 one **People & Access** surface per scope (Members·Pending·Teams·Roles·Access; invitations become Pending; inline role edit; access provenance) · SI4 Roles as a permission-matrix destination (custom-roles seam for **TG**) · SI5 end the tenant/person "account" collision (`/account`→`/you`). Relabel & regroup only — no tables, no tenancy change. Over **U**/**TH**/**TM**/**WID6**. |
