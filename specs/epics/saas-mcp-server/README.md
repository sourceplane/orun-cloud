...
| Field | Value |
|-------|-------|
| Status | **✅ Shipped (MCP0–MCP8) · unification phase In progress (MCP9–MCP10)** — the v1 epic is live: tool plane (25/25 budget, CI-guarded), both transports (stdio CLI + remote worker), OAuth 2.1 (D1 → Option A allow-list), resources/prompts, gated writes, metering/entitlement, console Connect page, and the MCP8 conformance + eval harness (`tests/mcp`). **Phase 2 (unification, D7)**: the *local* distribution moves to the **orun binary** — `orun mcp serve` serves work + platform tools natively in Go (paired spec: `orun/specs/orun-mcp/`, UM0–UM3); this repo owns MCP9 (tool-manifest export — ✅ shipped: committed `packages/mcp/tool-manifest.json`, freshness- + wire-parity-tested; `packages/mcp` is now the enforced contract source) and MCP10 (docs flip; node CLI labeled reference implementation). Deploy-lane follow-ups unchanged (`mcp.<domain>` hostname; live-stage E2E smokes); open product thread: **D3** (free-vs-paid) |
...
| MCP10 | **Unification phase (D7)** — docs flip: console Connect page, web-docs, and CLI README point at `orun mcp serve`; `orun-cloud mcp` labeled the node reference implementation | 🗓️ Planned (after orun UM2 merges; before/with the UM3 release) |
