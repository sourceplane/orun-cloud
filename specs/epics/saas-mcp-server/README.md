...
| Field | Value |
|-------|-------|
| Status | **✅ Shipped (MCP0–MCP10)** — the v1 epic is live: tool plane (25/25 budget, CI-guarded), both transports (stdio CLI + remote worker), OAuth 2.1 (D1 → Option A allow-list), resources/prompts, gated writes, metering/entitlement, console Connect page, and the MCP8 conformance + eval harness (`tests/mcp`). **Phase 2 (unification, D7) complete on this side**: the *local* distribution moved to the **orun binary** — `orun mcp serve` serves work + platform tools natively in Go (paired spec: `orun/specs/orun-mcp/`; UM0–UM2 shipped, **UM3 release in flight** — the only remaining unification item, orun-side); this repo shipped MCP9 (tool-manifest export: committed `packages/mcp/tool-manifest.json`, freshness- + wire-parity-tested; `packages/mcp` is the enforced contract source) and MCP10 (docs flip: console Connect page, web-docs, and CLI README point at `orun mcp serve`; the node CLI is the labeled reference implementation). Deploy-lane follow-ups unchanged (`mcp.<domain>` hostname; live-stage E2E smokes); open product thread: **D3** (free-vs-paid) |
...
| MCP9 | **Unification phase (D7)** — tool-manifest export: `packages/mcp` emits `tool-manifest.json` (names, schemas, annotations) as the enforced cross-implementation contract; freshness-tested; vendored by `orun` (UM1) | ✅ Shipped (committed `packages/mcp/tool-manifest.json` + `manifest` emitter; freshness byte-pinned in `@saas/mcp`, wire parity vs a real `tools/list` in `tests/mcp`) |
| MCP10 | **Unification phase (D7)** — docs flip: console Connect page, web-docs, and CLI README point at `orun mcp serve`; `orun-cloud mcp` labeled the node reference implementation | ✅ Shipped (orun binary primary everywhere — install + `orun auth login` + per-client snippets; node CLI demoted to a labeled reference-implementation block on all three surfaces; remote/OAuth content unchanged) |
