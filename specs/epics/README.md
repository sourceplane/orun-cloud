...
| [`saas-orun-backend-merge/`](./saas-orun-backend-merge/) | **BM** | Draft | state-worker, api-edge, identity-worker, integrations-worker, db/contracts, infra, tooling/migrate | Fold the standalone `orun-backend` hosted control plane (Durable-Object coordinator at `orun-api.sourceplane.ai`) into Orun Cloud's already-shipped `state-worker`: parity-harden the Postgres coordinator against the backend, add a backward-compatible legacy `/v1/runs` shim (unscoped `repository_id` → org/project), migrate the live data, cut over, and decommission. `orun-backend` is the reference, not a port. Extends **OP/OV**. |
