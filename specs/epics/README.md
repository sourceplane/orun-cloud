...
| [`saas-teams/`](./saas-teams/) | **TM** (account-owned principals over **WID**) | Draft | membership-worker, db, policy-engine, api-edge, contracts/sdk/cli, console | Teams as account-owned **principals** (à la GitHub org teams), granted roles via the existing `role_assignments` (`subject_type='team'`) and expanded into facts at authorization-context assembly — **no** policy-engine rewrite, **no** tenancy remodel. Account-scope grants cascade to all (incl. future) workspaces. Explicitly **not** a hierarchy level (that is `saas-workspace-id` Stage 2). Depends on **WID6** (account RBAC). |
