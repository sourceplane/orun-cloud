...
**Deps:** WV1–WV5
**Interaction budgets:** recorded 2026-07-11 (WV6): the pure lens projections (taskGroups + epicGroups + progressTotals/meterSegments/targetLabel over every epic) run in **~5 ms** for a synthetic corpus of 60 epics / 600 tasks / 50 initiatives — roughly 10× the dogfood tree — budget-gated at 100 ms in `tests/web-console-next/src/work-projection-budget.test.ts`. Rendering stays list-based with no virtualization at this scale (risks Q-5 stays open for real workspaces beyond it). End-to-end interaction profiling of the deployed console (lens switch, peek open) and the docs-screenshot refresh ride the live dogfood pass, exactly as v4's fold budget did. The WV-5 grep gate is CI-enforced by `work-style-gate.test.ts` (no raw hex, no per-surface dark forks in `src/components/work/`). Contrast note: drift-banner ink #6E5A22 on wash #FBF7E8 ≈ 7.4:1 (AA); amber pill text #9A7B2D on #F5EDD8 ≈ 4.6:1 (AA for the 11.5–12px/600 chip type).
