...
| [`teams-ownership/`](../teams-ownership/) | **TO** | Ownership | ✅ Shipped | The keystone: an **owner-handle → Team resolver** (account-authored map; **respects** the `18-state` *derived-never-authored* catalog invariant), catalog owner resolution + real `group-by: team`, and a **"My Teams / My Services"** lens across catalog + the flat Activities feed. |
