...
| D6 | Harness | Pluggable `AgentHarness`, Claude Code first (headless stream-JSON). Second-harness conformance is an AG9 exit criterion, keeping the seam honest. |
| D7 | Streaming/partition architecture | Per-session Durable Object as live relay + partition unit; append-only `session_events` + content-addressed R2 chunks as the durable record; DO reconstructible from storage. |
...
| **Cross-epic schedule coupling** — AG5–AG7 depend on WP1/WP2/WP5 and MCP0–MCP2 landing elsewhere. | The plan fronts AG0–AG4 (zero external dependencies, independently demoable); AG5+ tracks the WP/MCP epics' own registers rather than duplicating their milestones here. |
