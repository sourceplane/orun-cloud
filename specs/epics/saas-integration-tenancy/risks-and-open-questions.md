...
| A7 | **Sharing is account-only; inherited visibility is read-only** (ITX, IT9–IT12) | **Org kind** (`account`\|`workspace`\|`standalone`) is **derived** from `parent_org_id` (+ a child `COUNT`), never stored. Creating an `account`-scoped (shareable) connection and managing `share_mode`/grants is reserved to a **top-level (account/standalone) org**; a child is forced to `workspace` scope and `403`-ed from share/grant endpoints. This is an **org-position capability gate, not hierarchical RBAC** (A4 holds) — the same shape as IT8b gating management on ownership. A child workspace **sees** the account's `account`-scoped connection **read-only**, attributed *"Shared by «Account workspace»"* (the soft-visibility default D1 made concrete); it never gains write, and siblings' links stay invisible. The connection-list read is the first live use of the worker-side resolver (`resolveIntegrationParent`). |
