...
describe("buildPeopleTabs", () => {
  it("is Members, Pending, Access with Members as the bare default surface", () => {
    const tabs = buildPeopleTabs("acme");
    expect(tabs.map((t) => t.key)).toEqual(["members", "pending", "access"]);
    expect(tabs.map((t) => t.label)).toEqual(["Members", "Pending", "Access"]);
