...
function createFakeRepo(roleAssignments: RoleAssignment[] = []): MembershipRepository {
  return {
    ...
    async listPendingInvitationsByEmail() { return { ok: true, value: [] }; },
    ...
    async acceptInvitationById() { return { ok: false, error: { kind: "not_found" as const } }; },
...
function team(id: string, accountOrgId: string): Team {
  return {
    id, accountOrgId, name: "Platform", slugLower: "platform", handle: null, description: null, avatarRef: null, status: "active",
    createdAt: new Date("2026-01-01"), updatedAt: new Date("2026-01-01"),
  };
}
